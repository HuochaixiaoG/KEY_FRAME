...
from EffiV2S import efficientnetv2_s as create_model1
from ResNet import resnet50 as create_model2
from mobilenet import MobileNetV2 as create_model3
from Xception import xception as create_model4
from EfficientNet import efficientnet_b0 as create_model5
...
def main1(args):
    ...
    data_transform = {
        ...
        "train": transforms.Compose([transforms.Resize(img_size[num_model][0]),  # 进行随机裁剪成num_model大小
                                     ...
                                     transforms.RandomPerspective(distortion_scale=0.5, p=0.2, fill=0),  # 随机扭曲
    ...
    model = create_model1(num_classes=2).to(device)
    # summary(model, (3, 224, 224))
...
def main2(args):
    ...
    model = create_model2(num_classes=2).to(device)
    # summary(model, (3, 224, 224))
...
def main3(args):
    ...
    model = create_model3(num_classes=2).to(device)
    # summary(model, (3, 224, 224))
...
def main4(args):
    ...
    model = create_model4(num_classes=2).to(device)
    # summary(model, (3, 224, 224))
...
def main5(args):
    ...
    model = create_model5(num_classes=2).to(device)
    # summary(model, (3, 224, 224))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch-size', type=int, default=16)      # type是命令行参数应该被转换成的类型
    parser.add_argument('--lr', type=float, default=0.0001)
    parser.add_argument('--lrf', type=float, default=0.01)
    # 数据集所在根目录
    parser.add_argument('--data-path', type=str, default="G:\\Deepfakes_datasets\\test\\celeb-df-v1_opticalflow_weighted_keyframes_0.5")
    # 模型权重
